...
app.add_middleware(
    ...
    allow_headers=["Content-Type", "Accept-Language", "Authorization", "Cookie"],
)
...
async def authentication_error_handler(request: Request, exc: AuthenticationError):
    ...
    # Login qilinmagan yoki cookie yo‘q — kutiladigan 401 (/me, /refresh)
    path = request.url.path.rstrip("/")
    quiet_401 = exc.status_code == 401 and (
        path.endswith("/auth/refresh") or path.endswith("/auth/me")
    )
    if quiet_401:
        logger.debug("%s: %s", exc.__class__.__name__, exc.message)
    else:
...
# Include routers (/api/... va TZ bo'yicha /api/v1/... nusxalari)
