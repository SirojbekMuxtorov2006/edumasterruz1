...
@router.get(
    ...
    status_code=status.HTTP_200_OK,
    summary="Sessiya holati (anonim uchun ham 401 yo'q)",
)
async def get_session(
    ...
    """
    Cookie bo'lmasa yoki tokenlar yaroqsiz bo'lsa ham doim 200 va authenticated=false.
    Access eskirgan, refresh yaroqli bo'lsa yangi access cookie qo'yiladi.
    """
    if not session:
    ...
    # To'g'ridan-to'g'ri access orqali
    try:
    ...
    # Access yaroqsiz — refresh urinishi
    if not refresh_token_str:
        return SessionResponse(authenticated=False, user=None)
    try:
        new_access_token = await auth_service.refresh_access_token(refresh_token_str)
    ...
    try:
        payload = token_service.decode_token(new_access_token)
        user_id = int(payload.get("sub"))
    except (InvalidTokenError, ExpiredTokenError, ValueError, TypeError, KeyError):
        return SessionResponse(authenticated=False, user=None)
